...
class VCT0Executor(BaseExecutor):
    ...
    def configure_optimizers(self):
        ...
        """
        # TODO: Exercise 1.3
        # (1) make only the parameters of the mapping network trainable
        # hint: set the attribute `requires_grad` of the parameters (excluding the mapping network) in self.model.named_parameters() to False
        ##### Exercise 1.3 BEGIN ##### 
        print("here")
        #print([n for n, p in self.model.named_parameters()])
        exec(get_crib_code("1-3"))
        
        
        for n, p in self.model.named_parameters():
            if "clip_project" not in n:
                p.requires_grad = False
        print("Q1.3", [n for n, p in self.model.named_parameters()])
        ##### Exercise 1.3 END ##### 
    ...
    def training_step(self, sample_batched, batch_idx):
        #print("sample_batched", type(sample_batched))
        #print("length", len(sample_batched))
        #print("content", sample_batched.keys())
        # TODO: Exercise 1.4
        # prepare `train_batch` (EasyDict) with entries ‘clip_embeddings’ and ‘labels’
        # (1) Read `clip_embeddings` and `labels` from `sample_batched`
        # (2) Move them to GPU by .to(self.device). Refer to https://pytorch.org/docs/stable/generated/torch.Tensor.to.html for details.
        ##### Exercise 1.4 BEGIN #####
        clip_embeddings = sample_batched['clip_embeddings'].to(self.device)
        ...
        # self.crib_dict = {}
        # exec(get_crib_code("1-4"))
        # train_batch = self.crib_dict.get('train_batch', train_batch) # is no-op without cribs
        ##### Exercise 1.4 END #####
        ...
        self.log(
            "train/loss", batch_loss, on_step=True, on_epoch=True, logger=True, sync_dist=True
        )
    ...
    def _generative_step(self, sample_batched, batch_idx):
        ...
        batch_loss = forward_results.loss
        self.log(
            "test/loss", batch_loss, on_step=True, on_epoch=True, logger=True, sync_dist=True
        )
        ...
        for index, i in enumerate(labels):
            ...
            # (1) decode the model output sequence using self.decoder_tokenizer.decode(..), documentation: https://huggingface.co/docs/transformers/main_classes/tokenizer#transformers.PreTrainedTokenizerFast.decode 
            # (2) save the clean decoded output to the variable `decoded_output`. Note that you may want to remove special tokens.
            ##### Exercise 1.5 BEGIN #####
            decoded_output = self.decoder_tokenizer.decode(output_sequence, skip_special_tokens=True)
            ##### Exercise 1.5 END #####
            self.crib_dict = {}
            exec(get_crib_code("1-5"))
            decoded_output = self.crib_dict.get('decoded_output', decoded_output)
