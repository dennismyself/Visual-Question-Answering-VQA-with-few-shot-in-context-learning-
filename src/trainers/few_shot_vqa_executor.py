...
class FewShotVQAExecutor(BaseExecutor):
    ...
    def _generative_step(self, sample_batched, batch_idx):
        ...
        if self.config.data_loader.additional.ensemble_one_shots:
            ...
            outputs = self.model.generate(**generate_batch)
        ...
        for index, i in enumerate(labels):
            ...
            table_entry = [
                question_id,
                item["img_key_full"],
                sample_batched["in_context_img_keys"][index] if "in_context_img_keys" in sample_batched else None,
                item["question"],
            ...
            if i < 10:
                for table_entry in step_output["table_entries"]:
                    if self.config.args.log_prediction_tables_with_images:
                        ...
                        input_image_key = table_entry[1]
                        in_context_image_keys = table_entry[2]
                        input_image = artifact.get(f'COCO_val2014_{input_image_key}.jpg')
                        in_context_images = [
                            artifact.get(f'COCO_train2014_{k}.jpg') for k in in_context_image_keys
                        ]
                        table_entry[1] = input_image
                        table_entry[2] = in_context_images
