        ...
        for data_split, vqa_helper in vqa_helpers.items():
            ...
            if not self.data.vqa2_data[data_split]:
                ...
                for imgId, img_path in tqdm(img_list):
                    ...
                    #print("Q2.1",related_question_and_answers[0]["answers"])
                    for question_and_answer in related_question_and_answers:
                        ...
                        answers_cleaned = [i for i in question_and_answer['answers'].values() if i != ""]
                        entry_data = EasyDict({
                            'question': question_and_answer['question'], 
                            'question_id': question_and_answer['question_id'],
                            'answers': answers_cleaned,  # Extract unique answers
                            'gold_answer': most_frequent(answers_cleaned), 
                            'img_path': img_path, 
                            'img_key_full': img_key_full,
                            'img_key': img_key,
                            'img':img  # Placeholder for loading image data later if needed
                        })
