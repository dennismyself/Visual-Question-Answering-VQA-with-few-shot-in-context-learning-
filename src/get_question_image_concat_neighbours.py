...
def concat_embeddings_with_weights(df, embed_field_1, embed_field_2, alpha=1.0, beta=1.0):
    ...
    for i, row in df.iterrows():
        weighted_embed_1 = np.array(row[embed_field_1]) * alpha
        # weighted_embed_2 = np.array(row[embed_field_2]) * beta
        # concatenated = np.concatenate([weighted_embed_1, weighted_embed_2], axis=1)
        concatenated = weighted_embed_1
...
if __name__ == '__main__':
    ...
    # val_left = val_text_embeddings_df.merge(val_image_embeddings_df, how='left',  on = "question_id_prefix")
    # val_right = val_text_embeddings_df.merge(val_image_embeddings_df, how='right', on = "question_id_prefix")
    # val_inner = val_text_embeddings_df.merge(val_image_embeddings_df, how='inner', on = "question_id_prefix")

    # print("Q4.1 test", train_text_embeddings_df)
    # print("Q4.1 test col", train_text_embeddings_df.columns)
    # print("Q4.1 test len", train_text_embeddings_df.shape)
    
    # print("Q4.1 image", train_image_embeddings_df)
    # print("Q4.1 image col", train_image_embeddings_df.columns)
    # print("Q4.1 test len", train_text_embeddings_df.shape)

    # print("left", val_left["image_embedding"].isnull().sum())
    # print("right", val_right["image_embedding"].isnull().sum())
    # print("inner", val_inner["image_embedding"].isnull().sum())
    ...
    ##### Exercise 4.1 BEGIN ##### 
    val_df = val_text_embeddings_df.merge(val_image_embeddings_df, how='left', on = "question_id_prefix")
    train_df = train_text_embeddings_df.merge(train_image_embeddings_df, how='left', on = "question_id_prefix")
    # crib_dict = {}
