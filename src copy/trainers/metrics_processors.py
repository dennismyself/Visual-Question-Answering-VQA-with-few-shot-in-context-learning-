...
class MetricsProcessor:
    ...
    def compute_accuracy(self, module, data_dict, log_dict):
        ...
        for prediction in batch_predictions:
            ...
            annotation = self.data_loader.data.vqa_data.lookup.get(
                question_id, None
            )
            if annotation is None:
                logger.error(
                    f"Annotation not found for question_id: {question_id}"
                )
                raise ValueError(
    ...
    def compute_exact_match(self, module, data_dict, log_dict):
        ...
        for metric in exact_match_results.keys():
            exact_match_results[metric] = np.mean(
                np.array(exact_match_results[metric])
            )
    ...
    def compute_okvqa_scores(self, module, data_dict, log_dict) -> dict:
        ...
        try:
            ...
            for quesType in vqaEval.accuracy["perQuestionType"]:
                metrics_to_log[
                    f"accuracy_QuestionType_{quesType}"
                ] = vqaEval.accuracy["perQuestionType"][quesType]
            for ansType in vqaEval.accuracy["perAnswerType"]:
                metrics_to_log[
                    f"accuracy_AnswerType_{ansType}"
    ...
    def compute_vqa_scores(self, module, data_dict, log_dict) -> dict:
        """
        Compute VQA scores
        """
        try:
            ...
            # create vqa object and vqaRes object
            vqa_helper = self.data_loader.data.vqa_data.vqa_helpers[mode]
            vqaRes = vqa_helper.loadResFromDict(answers)
            ...
            if self.trainer.state.stage in ["sanity_check"]:
                logger.info(
                    f"Failed to compute VQA scores: {e}"
                    "This could be due to the fact that VQA parser requires all questions to evaluate"
                    "the accuracy. Ignore this error if this is the sanity check."
                )
            else:
    ...
    def write_predictions_to_file(self, module, data_dict, log_dict) -> dict:
        ...
        with open(os.path.join(self.config.results_path, "predictions.pkl"), "wb") as f:
