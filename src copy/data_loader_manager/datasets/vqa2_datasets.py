...
class VQA2Dataset(torch.utils.data.Dataset, ModuleParser):
    ...
    def collate_fn(self, batch):
        ...
        if self.config.args.log_prediction_tables_with_images:
            in_context_img_keys = [[item.img_key_full for item in sample.in_context_examples] for sample in batch]
        ...
        elif self.config.args.log_prediction_tables: 
            in_context_img_keys = [[str(item['img_key']).zfill(12) for item in sample['in_context_examples']] for sample in batch]
            batched_data = EasyDict(
                {
                    "question_ids": question_ids,
                    "questions": questions,
