...
class ModuleParser:
    ...
    def ClipEmbeddingInput(
        ...
        return EasyDict(
            clip_embeddings=torch.Tensor(sample.clip_embeddings)
        )
    ...
    def PostProcessClipEmbeddings(
        ...
        #print("imageurl shape after stack:", data_to_process.caption.shape)
        ...
        print("Q.03")
        print("clip_embeddings shape after stack:", post_processed_clip_embeddings.clip_embeddings.shape)
        
        #print("imageurl shape after stack:", len(post_processed_clip_embeddings.caption))
        return data_to_process
    ...
    def CaptionInput(self, sample, module):
        return EasyDict(
            {'caption': sample.get('caption')}
        )

    def PostProcessCaptionTokenization(self, data_to_process: EasyDict, module: EasyDict):
        captions = data_to_process.caption # a list of captions in the batch 
        ...
        print(captions)
         # TODO Exercise 1.2
         # (1) Use self.tokenizer() to initialise `tokenized_captions` from the list of captions in the batch
         # hint: the tokenizer is loaded as `self.tokenizer` (Why is this available?). 
         # You may refer to the PostProcessInputTokenization() function on the usage of self.tokenizer.
         # for details on the tokenizer, refer to https://huggingface.co/docs/transformers/v4.24.0/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase
        tokenized_captions = EasyDict({
            'input_ids': torch.Tensor([1,1,1,1,1]),
            'attention_mask': torch.Tensor([1,1,1,1,1]),
        })
        self.crib_dict = {}
        exec(get_crib_code("1-2"))
        tokenized_captions = self.crib_dict.get('tokenized_captions', tokenized_captions)
        print("Token")
        print(tokenized_captions)
        print("TokenEND")
        ##### Exercise 1.2 BEGIN ##### 
        # YOUR CODE: overwrite tokenized_captions to be the tokenized caption.
        #####  Exercise 1.2 END #####
        labels = tokenized_captions.input_ids
        labels[labels == self.tokenizer.pad_token_id] = -100
        labels_attention_mask = tokenized_captions.attention_mask

        ##### Sanity check BEGIN : Uncomment to see `labels`` can be detokenized back to ground-truth captions. #####
        #print("Sainty check")
        #print(self.tokenizer.batch_decode(labels, skip_special_tokens=True))
        #print("Sainty check  END")
        ##### Sanity check END #####
